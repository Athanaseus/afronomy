...
def predict_source(msname: str, ra: float = None, dec: float = None, flux: float = 1.0, column: str = "MODEL_DATA") -> None:
    ...
    #with xds_from_table(msname) as xds:
    #    print(xds.get('DATA').columns)
    #    if column not in xds.get('DATA').columns:
    #        xds.add_column(column, visibilities, dtype='complex64')
    #    else:
    #        xds.get('DATA')[column] = visibilities
    #    xds.write()
